...
class Seq2SeqLMOutputWithValue(ModelOutput):
    ...
    decoder_last_hidden_state: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
...
class ActorBartModel(BartPretrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.model = BartModel(config)
        # added to load all pretrained weights from 'ainize/bart-base-cnn'
    ...
    def forward(
        ...
        # value = self.value_head(decoder_outputs).squeeze(-1)

        if not return_dict:
            outputs = (lm_logits,) + outputs[1:] + (decoder_outputs,)  # + (value,)
            return outputs
